...
function normalizeXml(xml) {
  // remove espacos entre chaves e colapsa sequencias para evitar delimitadores quebrados
  return xml
    .replace(/\{\s+/g, "{")
    .replace(/\s+\}/g, "}")
    .replace(/\{ {0,}\{+/g, "{{")
...
function preparePdfAttachment(attachment, context = {}) {
  ...
  if (!isPdfBuffer(buffer)) {
    const ctx = Object.entries(context)
      ...
      .join(" ");
    console.warn("Anexo ignorado: buffer nao e PDF.", ctx);
    return null;
  }
  return {
...
`;
    ...
    try {
      ...
      console.warn("Falha ao limpar temp de conversao:", err.message || err);
    }
  }
}
  ...
  try {
    ...
    for (let idx = 0; idx < rows.length; idx += 1) {
      ...
      if (pdfValid) {
        ...
        console.warn(`PDF invalido gerado na linha ${idx + 1}.`);
      }
      ...
      if (emailColumn) {
        ...
        if (uniqueRecipients.length) {
          if (!pdfValid) {
            console.warn("PDF nao gerado; email nao enviado para", uniqueRecipients.join(", "));
          } else {
...
async function runSendJob(job, input) {
  ...
  try {
    ...
    const emailsToSend = Array.from(emailQueue.values());
    if (!emailsToSend.length) {
      let msg = "Nenhum destinatario encontrado.";
      if (recipientCount && pdfFailures) {
        msg = "PDF nao foi gerado. Verifique o LibreOffice no servidor.";
...
async function sendEmailsSmtp(queue, subject, message, smtpConfig, onProgress) {
  ...
  if (!options) {
    console.warn("SMTP nao configurado; emails nao enviados.");
    ...
    return { sent: 0, failed: queue.length, errors: ["SMTP nao configurado"], results };
  }
...
try {
  ...
  if ($fromAddress) {
    ...
    if (-not $account) {
      ...
      $list = ($available -join ", ")
      $errorMsg = "Conta Outlook nao encontrada: $fromAddress. Disponiveis: $list"
    ...
    if ($skipSend) { break }
    if (-not $item.to) {
      $report += [PSCustomObject]@{ to = $item.to; status = 'skipped'; error = 'Destinatario vazio'; filename = '' }
      continue
    }
    $attachments = @()
...
`;
    ...
    try {
      ...
      throw new Error(`Falha ao ler relatorio do Outlook: ${err.message || err}`);
    }
...
function renderDoc(templateBuffer, data, placeholders) {
  // Substituicao manual apenas (evita erros de delimitador do Docxtemplater)
  const cleaned = sanitizeTemplateBuffer(templateBuffer);
  const zip = new PizZip(cleaned);
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      console.error(err);
      res.status(500).json({ error: err.message || "Erro ao iniciar geracao" });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      for (let idx = 0; idx < rows.length; idx += 1) {
        const row = rows[idx];
        const data = {};
        // Preenche todos os placeholders encontrados com vazio para evitar erros de tag nao resolvida
        placeholders.forEach((ph) => {
          data[ph] = "";
        });
        const resolved = resolveRowValues(mapping, row);
